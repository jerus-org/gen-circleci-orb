...
| **T6** | **Tampering with committed CI wiring / releases.** | Commits and tags are **GPG-signed** (`release.toml`: `sign-tag`, `sign-commit`), a CI job **verifies commit signatures**, and release artifacts carry **SLSA/sigstore attestation** plus a signed binary tarball, with a documented verification process in [RELEASING.md](RELEASING.md). Satisfies R3. |
